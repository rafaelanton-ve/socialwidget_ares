(() => {
    ...
    const DEFAULT_CONFIG = {
        demo: {
            ...
            company_socials: { instagram: true, facebook: true, tiktok: true, youtube: true },
            socials: { whatsapp: true, telegram: true },
            social_links: {
    ...
    async function fetchTenantConfig(tenantId) {
        ...
        try {
            const response = await fetch(`${WIDGET_API_URL}?tenant=${encodeURIComponent(tenantId)}`, {
    ...
    async function initWidget() {
        ...
        style.textContent = `
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            .floating-button {
                ...
                cursor: pointer; box-shadow: var(--shadow-lg); z-index: 999999;
                display: flex; align-items: center; justify-content: center;
                transition: transform var(--transition), box-shadow var(--transition);
            }
            .floating-button:hover { transform: scale(1.06); box-shadow: 0 12px 40px rgba(0, 0, 0, 0.16); }
            .floating-button:active { transform: scale(0.96); }
            .overlay {
            ...
            .modal {
                ...
                max-height: calc(100vh - 140px); background: var(--bg-card);
            ...
            .modal.active { opacity: 1; visibility: visible; transform: translateY(0) scale(1); }
            .modal-header { display: flex; flex-direction: column; align-items: center; margin-bottom: 20px; position: relative; }
            .modal-logo { width: 56px; height: 56px; border-radius: 14px; object-fit: contain; background: #f5f5f5; padding: 8px; margin-bottom: 12px; }
            .modal-header-content { text-align: center; }
            .company-socials { display: flex; justify-content: center; gap: 12px; margin-bottom: 16px; }
            .company-social-link { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border-radius: 10px; background: #f5f5f5; color: var(--text-primary); text-decoration: none; transition: background var(--transition), color var(--transition), transform var(--transition); }
            .company-social-link:hover { background: var(--primary); color: white; transform: scale(1.1); }
            .company-name-row { display: flex; align-items: center; justify-content: center; gap: 6px; margin-bottom: 8px; }
            .company-name { font-size: 16px; font-weight: 600; color: var(--text-primary); letter-spacing: -0.01em; }
            .company-link { display: flex; align-items: center; color: var(--text-secondary); text-decoration: none; transition: color var(--transition), transform var(--transition); }
            .company-link:hover { color: var(--primary); transform: translateX(2px); }
            .close-button { position: absolute; top: 20px; right: 20px; width: 32px; height: 32px; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; border-radius: 8px; color: var(--text-secondary); transition: background var(--transition), color var(--transition); }
            .close-button:hover { background: var(--border); color: var(--text-primary); }
            .modal-title { font-size: 20px; font-weight: 600; color: var(--text-primary); letter-spacing: -0.02em; }
            ...
            .social-button { display: flex; align-items: center; gap: 14px; width: 100%; border: 1px solid var(--border); border-radius: 12px; padding: 16px; cursor: pointer; background: transparent; transition: background var(--transition), border-color var(--transition), transform var(--transition); }
            .social-button:hover { background: #fafafa; border-color: #d4d4d4; transform: translateX(4px); }
            .social-button:active { transform: translateX(4px) scale(0.98); }
            .social-icon { width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center; justify-content: center; background: #f5f5f5; color: var(--text-primary); transition: background var(--transition), color var(--transition); }
            .social-button:hover .social-icon { background: var(--primary); color: white; }
            .social-name { font-size: 15px; font-weight: 500; color: var(--text-primary); }
            @media (max-width: 420px) {
                .modal { width: calc(100vw - 32px); right: 16px; bottom: 90px; }
                .floating-button { bottom: 20px; right: 20px; }
            }
        `;

        const socialNames = { whatsapp: "WhatsApp", telegram: "Telegram", instagram: "Instagram", facebook: "Facebook", linkedin: "LinkedIn", mail: "Email", github: "GitHub" };
        ...
        const socialButtonsHTML = Object.entries(socials)
            ...
            .map(([icon]) => `
                <button class="social-button" data-url="${socialLinks[icon] || '#'}" aria-label="${socialNames[icon] || icon}">
                    <div class="social-icon">${SVG_ICONS[icon] || SVG_ICONS.chat}</div>
        ...
        wrapper.innerHTML = `
            <button class="floating-button" aria-label="Abrir chat">${SVG_ICONS.chat}</button>
            <div class="overlay"></div>
            <div class="modal">
                <button class="close-button" aria-label="Cerrar">${SVG_ICONS.close}</button>
                <div class="modal-header">
                    ${config.logo ? `<img src="${config.logo}" alt="Logo" class="modal-logo" />` : ""}
                    ${config.company_name ? `
                        <div class="company-name-row">
                            <span class="company-name">${config.company_name}</span>
                            <a href="${config.company_url || '#'}" target="_blank" class="company-link" aria-label="Visitar sitio web">${SVG_ICONS.external}</a>
                        </div>
                    ` : ""}
                    ${Object.keys(companySocials).length > 0 ? `
                        <div class="company-socials">
                            ${Object.entries(companySocials).filter(([_, active]) => active).map(([icon]) => `
                                <a href="${socialLinks[icon] || '#'}" target="_blank" class="company-social-link" aria-label="${icon}">${SVG_ICONS[icon] || ""}</a>
                            `).join("")}
                        </div>
                    ` : ""}
                ...
                <div class="social-list">${socialButtonsHTML}</div>
            </div>
        `;
        ...
        const openModal = () => { modal.classList.add("active"); overlay.classList.add("active"); };
        ...
        shadow.querySelectorAll(".social-button").forEach((button) => {
